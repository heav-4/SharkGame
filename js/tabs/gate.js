...
SharkGame.Gate = {
    ...
    messagePaidNotOpen: "For some reason, every slot is already filled. The structure doesn't open, though.<br/><strong>Perhaps it needs something else.</strong>",
            ...
            if (SharkGame.WorldTypes[world.worldType].gateRequirements.slots) {
                ...
                $.each(gate.requirements.slots, (resource, requiredAmount) => {
                    if (!gate.completedRequirements.slots[resource]) {
                        const resourceName = sharktext.getResourceName(resource, false, false, sharkcolor.getElementColor("tooltipbox", "background-color"));
                        SharkGame.Button.makeHoverscriptButton("gateCost-" + resource, "Insert " + sharktext.beautify(requiredAmount) + " " + resourceName + " into " + resourceName + " slot", buttonList, gate.onGateButton, gate.onHover, gate.onUnhover);
                    }
                });
            }
        }
    ...
    onHover() {
        ...
        if (amount < required) {
            button.html(`Need <span class='click-passthrough' style='color:#FFDE0A'>${sharktext.beautify(required - amount)}</span> more ${sharktext.getResourceName(resourceName, false, false, sharkcolor.getElementColor(button.attr("id"), "background-color"))} for ${sharktext.getResourceName(resourceName, false, false, sharkcolor.getElementColor(button.attr("id"), "background-color"))} slot`);
        }
    },
    onUnhover() {
        const gate = SharkGame.Gate;
