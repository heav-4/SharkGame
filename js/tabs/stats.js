...
SharkGame.Stats = {
    ...
    switchTo() {
        ...
        genStats.append($("<p>").html("Real time since you began your journey:<br/><span id='gameTime' class='timeDisplay'></span>").addClass("medDesc"));
        if (!firstTime) {
            genStats.append($("<p>").html("Relative time since you came through the gate:<br/><span id='runTime' class='timeDisplay'></span>").addClass("medDesc"));
    ...
    createDisposeButtons() {
        ...
        SharkGame.ResourceMap.forEach((_resource, resourceId) => {
            if (res.getTotalResource(resourceId) > 0 && stats.bannedDisposeCategories.indexOf(res.getCategoryOfResource(resourceId)) === -1) {
                SharkGame.Button.makeButton("dispose-" + resourceId, "Dispose of<br/>" +
                    sharktext.getResourceName(resourceId, false, false, sharkcolor.getElementColor("tooltipbox", "background-color")), buttonDiv, stats.onDispose);
            }
        });
    },
    updateDisposeButtons() {
        SharkGame.ResourceMap.forEach((_resource, resourceName) => {
            if (res.getTotalResource(resourceName) > 0 && stats.bannedDisposeCategories.indexOf(res.getCategoryOfResource(resourceName)) === -1) {
                ...
                let label = "Dispose of " +
                    sharktext.beautify(amountToDispose) +
                    "<br/>" +
                    sharktext.getResourceName(resourceName, disableButton, amountToDispose, sharkcolor.getElementColor("dispose-" + resourceName, "background-color"));
                if (amountToDispose <= 0) {
                    label =
                        ...
                        if (SharkGame.Settings.current.grottoMode === "advanced") {
                            ...
                            newValue =
                                "<span style='color:" +
                                    ...
                                    "'>x" +
                                    sharktext.beautify(res.getNetworkIncomeModifier("generator", resourceId) * res.getNetworkIncomeModifier("resource", incomeKey), false, 2) +
                                    "</span>";
                            if (cell.html() !== newValue.replace(/'/g, '"')) {
                                cell.html(newValue);
                            }
    ...
    createIncomeTable() {
        ...
        drawnResourceMap.forEach((headingData, headingName) => {
            ...
            resourceMapRow.append($("<td>")
                .html(sharktext.getResourceName(headingName, undefined, undefined, rowStyle === "evenRow" ? sharkcolor.getVariableColor("--color-dark") : sharkcolor.getVariableColor("--color-darker")))
                .attr("rowspan", subheadings)
                .addClass(rowStyle));
            ...
            $.each(headingData, (subheadingKey, subheadingValue) => {
                ...
                resourceMapRow.append($("<td>")
                    .html(sharktext.getResourceName(subheadingKey, undefined, undefined, rowStyle === "evenRow" ? sharkcolor.getVariableColor("--color-dark") : sharkcolor.getVariableColor("--color-darker")))
                    .addClass(rowStyle));
                const baseIncomeChangeChar = incomeValue *
                ...
                if (SharkGame.Settings.current.grottoMode === "advanced") {
                    addCell([
                        ...
                        baseIncomeChangeChar +
                            sharktext.beautify(SharkGame.ResourceMap.get(generatorName).baseIncome[incomeKey] / SharkGame.persistentFlags.dialSetting, false, 2) +
                            "/s",
                    ], "inline", "advanced-base-income-" + generatorName + "-" + incomeKey);
                    if (multipliers.upgrade || counter === 0) {
                        ...
                        if (upgradeMutiplier !== 1) {
                            addCell([res.UPGRADE_MULTIPLIER_COLOR, "x" + sharktext.beautify(upgradeMutiplier, false, 1)], multipliers.upgrade ? "inline" : undefined);
                        }
                        else
                    ...
                    if (multipliers.world || counter === 0) {
                        ...
                        if (worldMultiplier !== 1) {
                            addCell([res.WORLD_MULTIPLIER_COLOR, "x" + sharktext.beautify(worldMultiplier, false, 1)], multipliers.world ? "inline" : undefined);
                        }
                        else
                            addCell(undefined, multipliers.world ? "inline" : undefined);
                    }
                    if (multipliers.aspect || counter === 0) {
                        ...
                        if (aspectMultiplier !== 1) {
                            addCell([res.ASPECT_MULTIPLIER_COLOR, "x" + sharktext.beautify(aspectMultiplier, false, 1)], multipliers.aspect ? "inline" : undefined);
                        }
                        else
                            addCell(undefined, multipliers.aspect ? "inline" : undefined);
                    }
                    const resourceAffectMultiplier = res.getNetworkIncomeModifier("generator", generatorName, incomeValue) *
                        res.getNetworkIncomeModifier("resource", incomeKey, incomeValue);
                    if (resourceAffectMultiplier !== 1) {
                        addCell([res.RESOURCE_AFFECT_MULTIPLIER_COLOR, "x" + sharktext.beautify(resourceAffectMultiplier, false, 2)], "inline", "network-" + generatorName + "-" + incomeKey);
                    }
                    else
                    ...
                    addCell([
                        ...
                        baseIncomeChangeChar +
                            sharktext.beautify((incomeValue *
                                ...
                                SharkGame.persistentFlags.dialSetting, false, 2) +
                            "/s",
                    ], "inline", "base-income-" + generatorName + "-" + incomeKey);
                }
                addCell(undefined, "inline");
                if (SharkGame.Settings.current.incomeTotalMode === "percentage") {
                    ...
                    ], "inline", "income-" + generatorName + "-" + incomeKey);
                }
                else {
                    addCell([res.TOTAL_INCOME_COLOR, sharktext.boldString(changeChar + sharktext.beautifyIncome(realIncome))], "inline", "income-" + generatorName + "-" + incomeKey);
                }
                counter++;
    ...
    createTotalAmountTable() {
        ...
        SharkGame.ResourceMap.forEach((_resource, resourceId) => {
            if (res.getTotalResource(resourceId) > 0 && res.getCategoryOfResource(resourceId) !== "hidden") {
                const row = $("<tr>");
                row.append($("<td>").html(sharktext.getResourceName(resourceId, undefined, undefined, sharkcolor.getVariableColor("--color-darker"))));
                row.append($("<td>")
