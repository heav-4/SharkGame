...
SharkGame.Main = {
    ...
    wipeGame() {
        ...
        $("#versionNumber").html(`New Frontiers v ${SharkGame.VERSION} - ${SharkGame.VERSION_NAME}<br/>\
Mod of v ${SharkGame.ORIGINAL_VERSION}`);
    ...
    checkForUpdate() {
        $.getJSON("https://api.github.com/repos/Toby222/SharkGame/commits/alpha", (data) => {
            if (data.sha !== SharkGame.COMMIT_SHA) {
                $("#updateGameBox")
                    .html(`You see a new update swimming towards you.<br> On it you can just make out the words <br>"${data.commit.message.split("\n")[0]}". <br>Click to update.`)
                    .on("click", () => {
                    try {
                        SharkGame.Save.saveGame();
                        history.go(0);
    ...
    createBuyButtons(customLabel, addToWhere, appendOrPrepend, absoluteOnly) {
        ...
        _.each(SharkGame.Settings.buyAmount.options, (amount) => {
            ...
            buttonList.append($("<li>").append($("<button>")
        ...
        buttonList.append($("<li>").append($("<input>")
            .prop("type", "number")
