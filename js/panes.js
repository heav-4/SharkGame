...
SharkGame.Panes = {
    ...
    ending: "<p>Congratulations! You did it.<br/>You saved the sharks!</p>" +
        ...
        "<h3>Or are they?</h3>",
    cheats: "<p>You enter the space between worlds.</p>" +
    ...
    help: "<p>This game is a game about resources and discovery, and does not demand your full attention. " +
    ...
    donate: "<h2>You can donate to various projects and organizations below to help support sea life:</h2>" +
    ...
    notice: "<p>Welcome to the open <b>alpha</b> of v0.2 for New Frontiers.</p>" +
    ...
    safariNotice: "It seems you are using Safari.<br />This browser is currently not well-tested due to lack of corresponding hardware.<br />" +
...
SharkGame.PaneHandler = {
    ...
    nextPaneInStack() {
        ...
        if (panehandler.currentPane) {
            panehandler.showPane(panehandler.currentPane[0], panehandler.currentPane[1], panehandler.currentPane[2], panehandler.currentPane[3], panehandler.currentPane[4]);
        }
        else {
    ...
    setUpOptions() {
        ...
        $.each(categories, (category, settings) => {
            optionsTable.append($("<tr>").html("<h3><br><span style='text-decoration: underline'>" + sharktext.boldString(category) + "</span></h3>"));
        ...
        _.each(["1", "2", "3"], (tag) => {
            ...
            const loadButton = $("<button>")
                ...
                if (!$(`#load${tag}`).hasClass("disabled") && SharkGame.Save.savedGameExists(`Backup${tag}`)) {
                    if (confirm(`Are you absolutely sure you want to load this save${SharkGame.Save.getTaggedSaveCharacteristics(`Backup${tag}`)}?`)) {
        ...
        if (SharkGame.persistentFlags.unlockedDebug) {
            const loadButton = $("<button>")
                ...
                if (!$("#loadCheats").hasClass("disabled") && SharkGame.Save.savedGameExists("BackupCheats")) {
                    if (confirm(`Are you absolutely sure you want to load this save${SharkGame.Save.getTaggedSaveCharacteristics("BackupCheats")}?`)) {
        ...
        if (SharkGame.persistentFlags.unlockedDebug) {
            ...
            optionsTable.prepend($("<tr>").html("<h3><br><span style='text-decoration: underline'>" + sharktext.boldString("CHEATS and DEBUG") + "</span></h3>"));
        }
        row = $("<tr>");
        ...
        $.each(SharkGame.Keybinds.keybinds, (boundKey, boundAction) => {
            ...
            if (SharkGame.Keybinds.actions.includes(boundAction)) {
                ...
                _.each(SharkGame.Keybinds.actions, (potentialBoundAction, i) => {
                    selector.append(`<option${i % 2 === 0 ? ' class="evenMessage"' : ""} ${boundAction === potentialBoundAction ? " selected" : ""}>` +
