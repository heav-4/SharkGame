...
SharkGame.AspectTree = {
    ...
    init() {
        $.each(SharkGame.Aspects, (aspectId, aspectData) => {
            _.each(aspectData.prerequisites, (prerequisite) => {
                ...
                if (!sharkmisc.has(SharkGame.Aspects, prerequisite))
                    return;
    ...
    renderButton(context, posX, posY, width, height, icon = "general/missing-action", eventIcon = false, name) {
        ...
        if (icon !== null) {
            ...
            context.drawImage(eventIcon ? EVENT_SPRITE_SHEET : SPRITE_SHEET, sprite.frame.x, sprite.frame.y, sprite.frame.w, sprite.frame.h, posX, posY, width, height);
        }
    ...
    updateTooltip(button) {
        ...
        if (button === undefined) {
            ...
            if (button.level === 0) {
                ...
                const levelText = (button.core ? " core aspect" : "") + (button.core && button.noRefunds ? ", " : "") + (button.noRefunds ? "no refunds" : "");
                tooltipText =
    ...
    getTheoreticalRefundValue(aspect) {
        if (!aspect.level || aspect.noRefunds)
            return 0;
        let value = 0;
...
//# sourceMappingURL=aspecttree.js.map